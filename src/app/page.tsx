...
function OptionCard({ icon, title, description, onClick, highlighted = false }: OptionCardProps) {
  return (
    <button
      ...
      <div className='absolute left-8 top-[46px] flex h-12 max-w-[calc(100%-8rem)] items-start gap-5'>
        ...
        <div className='flex flex-col items-start gap-0.5 pt-0.5 text-left'>
          <p className="font-['Poppins',sans-serif] text-base font-semibold leading-6 text-white transition-colors group-hover:text-[#0a253a]">
            {title}
          </p>
          <p className="font-['Poppins',sans-serif] text-sm leading-5 text-white/50 transition-colors group-hover:text-black">
            {description}
          </p>
        </div>
      </div>
      <div className='absolute bottom-10 right-6'>
        <div className='h-6 w-6 overflow-hidden opacity-30 transition-all group-hover:opacity-100'>
...
export default function SimulateurStart() {
  ...
  return (
    <div className='relative min-h-screen w-full bg-[#0a253a] overflow-x-hidden'>
      ...
      {/* Main content */}
      <div className='mx-auto w-full max-w-[920px] px-4 py-[149px] sm:px-8'>
        <div className='flex w-full flex-col items-center gap-[62px]'>
          ...
          <div className='flex w-full flex-col items-center gap-[22px]'>
            ...
            <h1 className="font-['Poppins',sans-serif] text-center text-4xl font-extrabold leading-[70px] text-white sm:text-[56px] whitespace-pre-wrap">
              {`Quelle est votre `}
