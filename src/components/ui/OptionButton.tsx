...
export function OptionButton({
  ...
  const variantClasses = {
    default: 'hover:bg-white/12 active:bg-white/12',
    'hover-white':
      'hover:bg-white hover:shadow-[0px_10px_30px_-5px_rgba(0,0,0,0.3)] active:bg-white active:shadow-[0px_10px_30px_-5px_rgba(0,0,0,0.3)]',
    'hover-subtle': 'hover:bg-white/12 active:bg-white/12',
  }

  return (
    <button
      onClick={onClick}
      className={`group relative flex h-[136px] w-full cursor-pointer items-center rounded-xl bg-white/8 px-6 py-4 transition-all duration-300 hover:scale-[1.02] active:scale-[1.02] sm:px-8 lg:px-[35px] ${variantClasses[variant]} ${className}`}>
      <div className='flex items-center gap-4 sm:gap-5'>
        ...
        <div className='flex flex-col items-start gap-[3px] text-left'>
          <p
            className='text-sm font-semibold leading-6 text-white transition-colors group-hover:text-[#0a253a] group-active:text-[#0a253a] sm:text-base'
            style={{ fontFamily: typography.fonts.primary }}>
            {title}
          </p>
          <p
            className='text-xs leading-5 text-white/50 transition-colors group-hover:text-black group-active:text-black sm:text-sm'
            style={{ fontFamily: typography.fonts.primary }}>
            {description}
          </p>
        </div>
